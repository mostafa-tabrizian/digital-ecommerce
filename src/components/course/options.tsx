...
import { CourseLocation, Color, Size, Image } from '@prisma/client'
...
const Options = ({
   ...
   const [selectedColor, selectColor] = useState(course.courseLocation[0].color.color)
   const [selectedSize, selectSize] = useState(course.courseLocation[0].size.size)
   const [selectedLocation, selectFinalLocation] = useState(course.courseLocation[0])
   ...
   useEffect(() => {
      course.courseLocation.map((course) => {
         if (selectedColor == course.color.color && selectedSize == course.size.size)
            selectFinalLocation(course)
      })
   }, [selectedSize, course, selectedColor])

   const colors = () => {
      const list: string[] = []

      return course.courseLocation.map((course) => {
         if (course.quantity) {
            const color = course.color.color
      ...
      <div className='space-y-6'>
         <div className='grid grid-cols-2 gap-x-5 bg-white rounded-lg px-3 py-4'>
            <div className='from-slate-100 to-white shadow-sm bg-gradient-to-t p-2 rounded-lg'>
               <div className='flex space-x-1 mr-5 overflow-x-scroll styled-scrollbars'>
                  {course.courseLocation.map((course) => {
                     if (course.color.color !== selectedColor || !course.quantity) return

                     const size = course.size.size
         ...
         <div className='flex bg-white rounded-lg px-3 py-5 justify-between'>
            <div>
               ...
               <div style={{ fontSize: '2rem' }} className='font-bold toman_course'>
                  {selectedLocation.discount
                     ? (
