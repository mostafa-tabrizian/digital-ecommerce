...
const ImageInput = ({ selectedCourse }: { selectedCourse: string | null }) => {
   const [courseImages, setCourseImages] = useState<File[] | null>(null)
   const [loading, setLoading] = useState(false)

   const courseImagesMemo = useMemo(() => {
      return courseImages && Object.values(courseImages)
   }, [courseImages])

   const createS3 = async (imageName: string) => {
      try {
         const res = await fetch('/api/course/image/s3', {
            method: 'POST',
            body: JSON.stringify({
               imageName,
            }),
   ...
   const createDbData = async (key: string, imageName: string) => {
      ...
      try {
         const res = await fetch('/api/course/image/db', {
            method: 'POST',
   ...
   const onSubmit = async () => {
      if (!courseImages || !courseImagesMemo) {
      ...
      try {
         for (const image of courseImagesMemo) {
   ...
   return (
      <div>
         ...
         <div className='mt-3'>
            {courseImagesMemo &&
               courseImagesMemo.map((imageData: File) => {
                  return (
                     <Image
